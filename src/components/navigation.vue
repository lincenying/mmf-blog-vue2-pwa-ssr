    ...
    <nav class="global-nav">
        <div class="wrap">
            ...
            <div v-if="!backend" class="right-part">
                <span class="nav-search"><i class="icon icon-search-white"></i><input placeholder="记得按回车哦" class="nav-search-input" @keyup.enter="onSearch($event)"></span>
                <span v-if="isLogin" class="nav-me"><router-link to="/user/account" class="nav-me-link"><img :src="userEmail | avatar" class="nav-avatar-img"></router-link></span>
                <span v-else class="nav-me"><a href="javascript:;" class="nav-me-link" @click="handleLogin"><img :src="'noavatar' | avatar" class="nav-avatar-img"></a></span>
            </div>
        </div>
    </nav>
</template>
